...
class Media {
  ...
  update(
    ...
    // Show title only for the active (centered) card
    const distanceFromCenter = Math.abs(this.plane.position.x);
    const isActive = distanceFromCenter < this.width * 0.4; // Card is considered active if close to center
    if (this.title && this.title.mesh) {
      this.title.mesh.visible = isActive;
    }
