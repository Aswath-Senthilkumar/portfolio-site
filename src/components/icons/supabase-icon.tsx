...
export function SupabaseIcon({ className }: { className?: string }) {
  ...
  const gradientId1 = useMemo(
    () => `supabase-gradient-1-${Math.random().toString(36).slice(2, 11)}`,
    []
  );
  const gradientId2 = useMemo(
    () => `supabase-gradient-2-${Math.random().toString(36).slice(2, 11)}`,
    []
  );
  const clipPathId = useMemo(
    () => `supabase-clip-${Math.random().toString(36).slice(2, 11)}`,
    []
  );
